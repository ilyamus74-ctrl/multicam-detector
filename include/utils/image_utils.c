...
*/
/*
// CPU-fill вместо RGA colorfill (обход бага «RGA_MMU unsupported memory larger than 4G»)
	{
	    const int W = dst->width;
	    const int H = dst->height;
	    const unsigned char c = (unsigned char)bg_color; // обычно 114
	    unsigned char* p = dst->virt_addr;
	    for (int y = 0; y < H; ++y) {
    		unsigned char* row = p + y * W * 3;
    		for (int x = 0; x < W; ++x) {
        	    row[3*x+0] = c;
        	    row[3*x+1] = c;
        	    row[3*x+2] = c;
...
*/
    ...
    // CPU-fill вместо RGA colorfill (обход бага «RGA_MMU unsupported memory larger than 4G»)
    size_t dst_size = get_image_size(dst_img);
    if (dst_img->virt_addr && dst_size > 0) {
        memset(dst_img->virt_addr, color, dst_size);
	}
    }
